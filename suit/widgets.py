...
class EnclosedInput(TextInput):
    ...
    def enclose_value(self, value, wrapper_class):
        ...
        return '<span class="input-group-%s"%s>%s</span>' % (
            wrapper_class,
            f"onclick={self.onclick_append}" if self.onclick_append else "",
            value,
        )
    ...
    def render(self, name, value, attrs=None, renderer=None):
        ...
        if self.prepend:
            ...
            output = '<span class="input-group-text">%s</span>%s' % (
                self.prepend,
                output,
            )
        if self.append:
            self.append = self.enclose_value(self.append, self.append_class)
            output = '%s<span class="input-group-text">%s</span>' % (
                output,
                self.append,
            )
